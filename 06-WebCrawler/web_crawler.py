...
def web_crawler(start_url, num_links):
    ...
    def crawl(url):
        ...
        while len(local_sites) < 5 and i < len(links) and len(relevant_sites) < num_links:
            ...
            if link_str.startswith('http') and link_str.find('space') != -1 and not any(
                    link_str.find(link) != -1 for link in
                    ['twitter', 'facebook', 'pinterest', 'youtube', 'reddit', 'instagram', 'flickr', 'shop',
                     'subscribe', 'images']):
                try:
...
def main():
    ...
    top_key_terms = ['webb', 'dart', 'falcon', 'asteroid', 'didymos', 'dimorphos', 'spacex', 'rover', 'russia',
                     'rocket']
